...
- the derived `selected_values`: the selected factor per development period followed by the selected tail, at full precision, which is the chain `ultimate_vector` and the percentage developed use
...
A persisted number that anything calculates with is stored at the precision it was observed or computed with and is never rounded: the observed input triangle (`input_data_triangle_values`), the average formula `values`, the Curves tab's `selected_values`, the Ratio Basis, and the `ultimate_vector`. Any fixed decimal place is the wrong rule for the operands every ratio and every average divides, because how much of a number it keeps depends only on how large that number happens to be — ten decimals was generous for a loss figure and still too coarse for a near-zero "% of" figure, where the trimmed tail moved a ratio read at four — and a six-decimal factor multiplied ten times through the cumulative chain moved an ultimate by tenths at the least-mature origin. A JSON number round-trips a double exactly and the shortest text that reads back as the same value is what lands on disk, so the file stays as readable as it was. The ratio triangle, which nothing chains, is normalized half-away-from-zero at six decimals so a reader can check it by eye, and the input source revision is still fingerprinted at six so a stored revision does not shift under this rule. The page writes these values the same way: `dfm_persistence.js` rounds only the ratio triangle, so a method saved from the browser and one recalculated by the Engine hold the same numbers. The canonical Python contract owns ratio calculation, average calculation, internal User Entry formula evaluation, ultimate calculation, field projection, and revisions. The frontend calls the local preview endpoint for canonical interactive derivation.
