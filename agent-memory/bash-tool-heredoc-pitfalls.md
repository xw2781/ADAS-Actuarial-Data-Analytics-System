...
metadata: 
  ...
  modified: 2026-09-09T13:47:55.766Z
---
...
Cheapest repair after a `sed -i` slip (used 2026-09-09): `sed -i 's/\r*$/\r/' <files>` puts exactly one CR back on every line, and `file <path>` then reports "with CRLF line terminators"; `git diff --stat` stays at the intended lines because the index is LF.
